...
func (mt *MPTTracer) GetContract(addr types.Address) (*types.SmartContract, error) {
	...
	// TODO: add test for this, it was a long debug
...
func getStorageTraces[V any, VPtr execution.MPTValue[V]](
...
func (mt *MPTTracer) getStorageTraces(initialRoot common.Hash, currentRoot common.Hash) ([]StorageTrieUpdateTrace, error) {
	rawMpt := mpt.NewDbMPT(mt.rwTx, mt.shardId, db.StorageTrieTable)
	return getStorageTraces(rawMpt, execution.NewStorageTrie, initialRoot, currentRoot)
}

// getAccountTrieTraces retrieves traces for changes in a contract trie.
func (mt *MPTTracer) getAccountTrieTraces(initialRoot common.Hash, currentRoot common.Hash) ([]ContractTrieUpdateTrace, error) {
	rawMpt := mpt.NewDbMPT(mt.rwTx, mt.shardId, db.ConfigTrieTable)
	return getStorageTraces(rawMpt, execution.NewContractTrie, initialRoot, currentRoot)
}
